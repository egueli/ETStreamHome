...
//    }
    ...
    private Response respond(Map<String, String> headers, IHTTPSession session, String uri) {
        ...
        for (int i = 0; !canServeUri && i < roots.size(); i++) {
            homeDir = roots.get(i);
            canServeUri = true; //canServeUri(uri, homeDir);
        }
        if (!canServeUri) {
...
//            response = serveFile(uri, headers, f, mimeTypeForFile);
            response = serveStream(uri, headers, f, mimeTypeForFile);
//        }
...
//    }
    ...
    Response serveStream(String uri, Map<String, String> header, File file, String mime) {
        Response res;
        try {
